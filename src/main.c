...
gint play_iter(GtkTreeIter * playiter, gint restart_second)
{
    ...
    /*
       ...
       gtk_window_set_title(GTK_WINDOW(dialog), "MATE MPlayer Error");
    ...
    if (control_id == 0 && !streaming_media(uri)) {
        ...
        g_object_unref(file);
        recent_data->app_name = g_strdup("mate-mplayer");
        recent_data->app_exec = g_strdup("mate-mplayer %u");
...
int main(int argc, char *argv[])
{
    ...
    uri = g_strdup_printf("%s/mate-mplayer/plugin", g_get_user_config_dir());
    if (!g_file_test(uri, G_FILE_TEST_IS_DIR)) {
        g_mkdir_with_parents(uri, 0775);
    }
    g_free(uri);
    ...
    default_playlist = g_strdup_printf("file://%s/mate-mplayer/default.pls", g_get_user_config_dir());
    ...
    enable_caja_plugin = gm_pref_store_get_boolean_with_default(gm_store, ENABLE_CAJA_PLUGIN, TRUE);
    if (mplayer_bin == NULL)
    ...
    context = g_option_context_new(_("[FILES...] - MATE Media player based on MPlayer"));
#ifdef GTK2_12_ENABLED
