...
To send command to ALL running mate-mplayers (multihead applications)
dbus-send  --type=signal / com.mate.mplayer.Play string:'http://www.hotmail.com/playfile.asx'


When windowid is not specified
dbus-send  --type=signal /pid/[pid] com.mate.mplayer.Play string:'http://www.hotmail.com/playfile.asx'


When windowid is specified
dbus-send  --type=signal /window/[windowid] com.mate.mplayer.Play string:'http://www.hotmail.com/playfile.asx'

When controlid is specified
dbus-send  --type=signal /control/[controlid] com.mate.mplayer.Play string:'http://www.hotmail.com/playfile.asx'
...
static DBusHandlerResult filter_func(DBusConnection * connection, DBusMessage * message, void *user_data)
{
    ...
    if (dbus_message_get_path(message)) {
        ...
        if (g_ascii_strcasecmp(dbus_message_get_path(message), "/") == 0 ||
            ...
            g_ascii_strcasecmp(dbus_message_get_path(message), "/org/mate/SettingsDaemon") == 0 ||
            g_ascii_strcasecmp(dbus_message_get_path(message), "/org/mate/SettingsDaemon/MediaKeys") == 0) {
...
*/
                ...
                if (dbus_message_is_method_call(message, "org.freedesktop.DBus.Introspectable", "Introspect")) {
                    ...
                    xml = g_string_append(xml,
                                          "<interface name=\"com.mate.mplayer\">\n"
                                          "    <method name=\"Test\">\n"
                                          "    </method>\n"
                ...
                if (dbus_message_is_method_call(message, "com.mate.mplayer", "Test")) {
                    reply_message = dbus_message_new_method_return(message);
                    dbus_connection_send(connection, reply_message, NULL);
                    dbus_message_unref(reply_message);
                    return DBUS_HANDLER_RESULT_HANDLED;
                }
                if (dbus_message_is_method_call(message, "com.mate.mplayer", "GetVolume")) {
                    reply_message = dbus_message_new_method_return(message);
                ...
                if (dbus_message_is_method_call(message, "com.mate.mplayer", "GetFullScreen")) {
                    reply_message = dbus_message_new_method_return(message);
                ...
                if (dbus_message_is_method_call(message, "com.mate.mplayer", "GetShowControls")) {
                ...
                if (dbus_message_is_method_call(message, "com.mate.mplayer", "GetTime")) {
                    reply_message = dbus_message_new_method_return(message);
                ...
                if (dbus_message_is_method_call(message, "com.mate.mplayer", "GetDuration")) {
                    reply_message = dbus_message_new_method_return(message);
                ...
                if (dbus_message_is_method_call(message, "com.mate.mplayer", "GetPercent")) {
                    reply_message = dbus_message_new_method_return(message);
                    percent =
                ...
                if (dbus_message_is_method_call(message, "com.mate.mplayer", "GetCacheSize")) {
                    reply_message = dbus_message_new_method_return(message);
                ...
                if (dbus_message_is_method_call(message, "com.mate.mplayer", "GetPluginAudioCacheSize")) {
                    reply_message = dbus_message_new_method_return(message);
                    dbus_message_append_args(reply_message, DBUS_TYPE_INT32,
                ...
                if (dbus_message_is_method_call(message, "com.mate.mplayer", "GetPluginVideoCacheSize")) {
                    reply_message = dbus_message_new_method_return(message);
                    dbus_message_append_args(reply_message, DBUS_TYPE_INT32,
                                             &plugin_video_cache_size, DBUS_TYPE_INVALID);
                    dbus_connection_send(connection, reply_message, NULL);
                ...
                if (dbus_message_is_method_call(message, "com.mate.mplayer", "GetPlayState")) {
                    reply_message = dbus_message_new_method_return(message);
                ...
                if (dbus_message_is_method_call(message, "com.mate.mplayer", "GetBitrate")) {
                    dbus_error_init(&error);
                    if (dbus_message_get_args(message, &error, DBUS_TYPE_STRING, &s, DBUS_TYPE_INVALID)) {
                ...
                if (dbus_message_is_method_call(message, "com.mate.mplayer", "GetURI")) {
                    reply_message = dbus_message_new_method_return(message);
                ...
                if (dbus_message_is_method_call(message, "com.mate.mplayer", "GetTitle")) {
                    reply_message = dbus_message_new_method_return(message);
                ...
                if (dbus_message_is_method_call(message, "com.mate.mplayer", "GetCachePercent")) {
                    reply_message = dbus_message_new_method_return(message);
...
void dbus_open(gchar * arg)
{
    ...
    if (connection != NULL) {
        path = g_strdup_printf("/");
        if (replace_and_play) {
            message = dbus_message_new_signal(path, "com.mate.mplayer", "Open");
        } else {
            message = dbus_message_new_signal(path, "com.mate.mplayer", "Add");
        }
...
void dbus_set_playlist_name(gchar * arg)
{
    ...
    if (connection != NULL) {
        path = g_strdup_printf("/");
        message = dbus_message_new_signal(path, "com.mate.mplayer", "SetPlaylistName");
        localarg = g_strdup(arg);
        dbus_message_append_args(message, DBUS_TYPE_STRING, &localarg, DBUS_TYPE_INVALID);
        dbus_connection_send(connection, message, NULL);
        dbus_message_unref(message);
...
void dbus_send_rpsignal(gchar * signal)
{
    ...
    if (connection != NULL) {
        ...
        message = dbus_message_new_signal(path, "com.mate.mplayer", localsignal);
        dbus_message_append_args(message, DBUS_TYPE_INT32, &id, DBUS_TYPE_INVALID);
        dbus_connection_send(connection, message, NULL);
        dbus_message_unref(message);
        g_free(localsignal);
...
gboolean dbus_hookup(gint windowid, gint controlid)
{
    ...
    // dbus_bus_request_name(connection,"com.mate.mplayer",0,NULL);
    // dbus_connection_register_object_path (connection,"/com/mate/mplayer", &vtable,NULL);

    match = g_strdup_printf("type='signal',interface='com.mate.mplayer'");
    ...
    match = g_strdup_printf("type='signal',interface='org.mate.SettingsDaemon'");
    dbus_bus_add_match(connection, match, &error);
    if (verbose)
        printf("Using match: %s\n", match);
    g_free(match);
    dbus_error_free(&error);
    if (use_mediakeys) {
        match = g_strdup_printf("type='signal',interface='org.mate.SettingsDaemon.MediaKeys'");
        dbus_bus_add_match(connection, match, &error);
        if (verbose)
            printf("Using match: %s\n", match);
        g_free(match);
    ...
    if (control_id != 0) {
        path = g_strdup_printf("com.mate.mplayer.cid%i", control_id);
        ...
        ret = dbus_bus_request_name(connection, "com.mate.mplayer", 0, NULL);
...
#if SM_INHIBIT
                message =
                    dbus_message_new_method_call("org.mate.SessionManager",
                                                 "/org/mate/SessionManager", "org.mate.SessionManager", "UnInhibit");
...
#if SS_INHIBIT
                message =
                    dbus_message_new_method_call("org.mate.ScreenSaver", "/org/mate/ScreenSaver",
    ...
    if (screensaver_disabled == FALSE) {
        if (use_xscrnsaver) {
            ...
            if (connection != NULL) {
                ...
                message =
                    dbus_message_new_method_call("org.mate.ScreenSaver", "/org/mate/ScreenSaver",
                                                 "org.mate.ScreenSaver", "Inhibit");
                app = g_strdup_printf("mate-mplayer");
                ...
                message =
                    dbus_message_new_method_call("org.mate.SessionManager",
                                                 "/org/mate/SessionManager", "org.mate.SessionManager", "Inhibit");
                app = g_strdup_printf("mate-mplayer");
                reason = g_strdup_printf("playback");
                flags = 8;
