...
#include "support.h"
#include "common.h"
#include "../pixmaps/mate_mplayer.xpm"
/*
...
static GtkWidget *config_enable_caja_plugin;
...
gboolean show_copyurl(void *data)
{
    ...
    if (control_id == 0) {
        buf = g_strdup_printf(_("%s - MATE MPlayer"), gmtk_media_player_get_uri(GMTK_MEDIA_PLAYER(media)));
    ...
    if (data != NULL && idle != NULL && gmtk_get_visible(window)) {
        ...
        if (total > 1) {
            if (name == NULL || strlen(name) < 1) {
                buf = g_strdup_printf(_("(%i/%i) - MATE MPlayer"), current + 1, total);
            } else {
                buf = g_strdup_printf(_("%s - (%i/%i) - MATE MPlayer"), name, current + 1, total);
            }
        } else {
            if (name == NULL || strlen(name) < 1) {
                buf = g_strdup_printf(_("MATE MPlayer"));
            } else {
                buf = g_strdup_printf(_("%s - MATE MPlayer"), name);
            }
        }
        gtk_window_set_title(GTK_WINDOW(window), buf);
        g_free(buf);
...
gboolean set_media_label(void *data)
{
    ...
    if (idle->fromdbus == FALSE) {
        ...
        if (show_notification && control_id == 0 && !gtk_window_is_active((GtkWindow *) window)) {
            notify_init("mate-mplayer");
#ifdef NOTIFY0_7_ENABLED
            notification = notify_notification_new(idle->display_name, idle->media_notification, "mate-mplayer");
#else
            notification = notify_notification_new(idle->display_name, idle->media_notification, "mate-mplayer", NULL);
#endif
#ifdef GTK2_12_ENABLED
...
{
    gm_store = gm_pref_store_new("mate-mplayer");
...
void recent_manager_changed_callback(GtkRecentManager * recent_manager, gpointer data)
{
    ...
    gtk_recent_filter_add_application(recent_filter, "mate-mplayer");
...
#endif
#endif
    gtk_show_about_dialog(GTK_WINDOW(window), "name", _("MATE MPlayer"), "authors", authors,
                          "copyright", "Copyright © 2007-2011 Kevin DeKorte", "comments",
                          _("A media player for MATE that uses MPlayer"), "version", VERSION,
                          "license",
                          _
                          ("Mate MPlayer is free software; you can redistribute it and/or modify it under\nthe terms of the GNU General Public License as published by the Free\nSoftware Foundation; either version 2 of the License, or (at your option)\nany later version."
                           "\n\nMate MPlayer is distributed in the hope that it will be useful, but WITHOUT\nANY WARRANTY; without even the implied warranty of MERCHANTABILITY or\nFITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for\nmore details."
                           "\n\nYou should have received a copy of the GNU General Public License along with\nMate MPlayer if not, write to the\n\nFree Software Foundation, Inc.,\n51 Franklin St, Fifth Floor\nBoston, MA 02110-1301 USA")


                          ,
                          "website", "http://code.google.com/p/mate-mplayer/",
                          "translator-credits",
...
void config_apply(GtkWidget * widget, void *data)
{
    ...
    enable_caja_plugin = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(config_enable_caja_plugin));
#ifdef NOTIFY_ENABLED
    ...
    if (GTK_IS_STATUS_ICON(status_icon)) {
        ...
        if (show_status_icon) {
            GtkIconTheme *icon_theme = gtk_icon_theme_get_default();
            if (gtk_icon_theme_has_icon(icon_theme, "mate-mplayer")) {
                status_icon = gtk_status_icon_new_from_icon_name("mate-mplayer");
            } else {
    ...
    gm_pref_store_set_boolean(gm_store, ENABLE_CAJA_PLUGIN, enable_caja_plugin);
...
void menuitem_config_callback(GtkMenuItem * menuitem, void *data)
{
    ...
    gtk_window_set_title(GTK_WINDOW(config_window), _("MATE MPlayer Configuration"));
...
//    gtk_widget_set_sensitive(config_subtitle_shadow, disable_embeddedfonts);
    ...
    config_single_instance = gtk_check_button_new_with_label(_("Only allow one instance of Mate MPlayer"));
    ...
    config_enable_caja_plugin = gtk_check_button_new_with_label(_("Enable Caja Plugin"));
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(config_enable_caja_plugin), enable_caja_plugin);
    gtk_table_attach(GTK_TABLE(conf_table), config_enable_caja_plugin, 0, 2, i, i + 1, GTK_FILL, GTK_SHRINK, 0, 0);
    i++;
    ...
    gtk_widget_set_tooltip_text(config_softvol,
                                _("Set this option if changing the volume in Mate MPlayer changes the master volume"));
#else
    tooltip = gtk_tooltips_new();
    gtk_tooltips_set_tip(tooltip, config_softvol,
                         _("Set this option if changing the volume in Mate MPlayer changes the master volume"), NULL);
#endif
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(config_softvol), softvol);
    ...
    case ATTRIBUTE_ALBUM:
        ...
        if (gmtk_media_player_get_attribute_string(GMTK_MEDIA_PLAYER(media), ATTRIBUTE_TITLE)) {
            ...
            if (metadata->title != NULL && strlen(metadata->title) > 0) {
                title = g_strdup_printf(_("%s - MATE MPlayer"), metadata->title);
...
void player_error_message_callback(GmtkMediaPlayer * media, gchar * message)
{
    ...
    if (control_id == 0) {
        ...
        gtk_window_set_title(GTK_WINDOW(dialog), g_dgettext(GETTEXT_PACKAGE, "MATE MPlayer Error"));
        gtk_dialog_run(GTK_DIALOG(dialog));
        gtk_widget_destroy(dialog);
    } else {
        gm_log(TRUE, G_LOG_LEVEL_MESSAGE, "MATE MPlayer Error: %s", message);
    }
}
...
void make_button(gchar * src, gchar * hrefid)
{
    ...
    if (error != NULL) {
        g_error_free(error);
        error = NULL;
        basepath = g_strdup_printf("%s/mate-mplayer/plugin", g_get_user_cache_dir());
        dirname = gm_tempname(basepath, "mate-mplayerXXXXXX");
...
GtkWidget *create_window(gint windowid)
{
    ...
    gtk_window_set_title(GTK_WINDOW(window), _("MATE MPlayer"));
    ...
    icon_list = NULL;
    if (gtk_icon_theme_has_icon(icon_theme, "mate-mplayer")) {
        pb_icon = gtk_icon_theme_load_icon(icon_theme, "mate-mplayer", 128, 0, NULL);
        if (pb_icon)
            icon_list = g_list_prepend(icon_list, pb_icon);
        pb_icon = gtk_icon_theme_load_icon(icon_theme, "mate-mplayer", 64, 0, NULL);
        if (pb_icon)
            icon_list = g_list_prepend(icon_list, pb_icon);
        pb_icon = gtk_icon_theme_load_icon(icon_theme, "mate-mplayer", 48, 0, NULL);
        if (pb_icon)
            icon_list = g_list_prepend(icon_list, pb_icon);
        pb_icon = gtk_icon_theme_load_icon(icon_theme, "mate-mplayer", 24, 0, NULL);
        if (pb_icon)
            icon_list = g_list_prepend(icon_list, pb_icon);
        pb_icon = gtk_icon_theme_load_icon(icon_theme, "mate-mplayer", 22, 0, NULL);
        if (pb_icon)
            icon_list = g_list_prepend(icon_list, pb_icon);
        pb_icon = gtk_icon_theme_load_icon(icon_theme, "mate-mplayer", 16, 0, NULL);
        if (pb_icon)
            icon_list = g_list_prepend(icon_list, pb_icon);
    } else {
        pb_icon = gdk_pixbuf_new_from_xpm_data((const char **) mate_mplayer_xpm);
        if (pb_icon)
            icon_list = g_list_prepend(icon_list, pb_icon);
    }
    ...
    if (control_id == 0 && show_status_icon) {
        if (gtk_icon_theme_has_icon(icon_theme, "mate-mplayer-panel")) {
            status_icon = gtk_status_icon_new_from_icon_name("mate-mplayer-panel");
        } else if (gtk_icon_theme_has_icon(icon_theme, "mate-mplayer")) {
            status_icon = gtk_status_icon_new_from_icon_name("mate-mplayer");
        } else {
            status_icon = gtk_status_icon_new_from_pixbuf(pb_icon);
        }
