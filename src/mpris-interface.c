...
gboolean mpris_hookup(gint controlid)
{
    ...
    path = g_strdup_printf("org.mpris.MediaPlayer2.mate-mplayer");
    ret = dbus_bus_request_name(mpris_connection, path, 0, NULL);
    g_free(path);

    if (control_id != 0) {
        path = g_strdup_printf("org.mpris.MediaPlayer2.mate-mplayer.cid%i", control_id);
        ret = dbus_bus_request_name(mpris_connection, path, 0, NULL);
        g_free(path);
    }
